...
module.exports.viewCart = async (req, res) => {
  ...
  try {
    ...
    const cartItems = await Promise.all(
      user.cart.map(async (item) => {
        ...
        try {
          ...
          let earlyBirdSlot = classInfo.earlyBirdSlot || 0;

          const participantsWithPrice = item.participants.map((participant) => {
            let price = item.courseId.price || 0;
            let discount_type = "";

            if (participant.isAlumni && item.courseId.alumniPrice) {
              price = item.courseId.alumniPrice;
              discount_type = "Alumni";
            } else if (earlyBirdSlot > 0 && item.courseId.earlyBirdPrice) {
              price = item.courseId.earlyBirdPrice;
              discount_type = "Early Bird";
              earlyBirdSlot--;
            } else if (
              item.participants.length > 1 &&
              item.courseId.bundlePrice
            ) {
          ...
          return {
            ...
            classInfo: {
              ...
              earlyBirdSlot: classInfo.earlyBirdSlot || 0,
            },
            image: imageBase64,
    ...
    // Filter out null items and handle empty cart
