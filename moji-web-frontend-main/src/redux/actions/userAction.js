...
export const updateReferralStatus = (referrerId, referredId) => async (dispatch) => {
	try {
		dispatch(setLoader(true));
		const { data } = await axios.put(
			process.env.REACT_APP_API_URL + `/api/v1/referral/${referrerId}/${referredId}`, 
			{}, 
		...
		if (data.success) {
			// Handle success case
			dispatch(setSuccess("Nhập mã thành công, chúc mừng bạn đã trở thành thành viên Premium!"));
