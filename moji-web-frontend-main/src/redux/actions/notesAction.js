...
import { setDetailsLoader, setNotes, updateNotes, removeNote, addNote, setUsersWithNotesCount } from '../slices/notesSlice';
import { setSuccess, setError } from '../slices/appSlice';

export const createNote = (noteData) => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.post(process.env.REACT_APP_API_URL + `/api/v1/notes`, noteData, {
...
export const getNotes = () => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.get(process.env.REACT_APP_API_URL + `/api/v1/notes`, {
            withCredentials: true,
        });
...
export const getNote = (id) => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.get(process.env.REACT_APP_API_URL + `/api/v1/notes/${id}`, {
            withCredentials: true,
        });
        dispatch(setDetailsLoader(false));
    } catch (err) {
...
export const updateNote = (passageId, passageData) => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.put(process.env.REACT_APP_API_URL + `/api/v1/notes/${passageId}`, passageData, {
            withCredentials: true,
        });
...
export const getNotesByUserId = (userId) => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.get(process.env.REACT_APP_API_URL + `/api/v1/user/${userId}`, {
            withCredentials: true,
        });
        dispatch(setNotes(data.data));
        dispatch(setDetailsLoader(false));
...
export const getUsersWithNotesCount = () => async (dispatch) => {
    try {
        dispatch(setDetailsLoader(true));
        const { data } = await axios.get(process.env.REACT_APP_API_URL + `/api/v1/counter`, {
            withCredentials: true,
        });
        // console.log(data)
