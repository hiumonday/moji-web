...
const WideCard = ({ post }) => {
    ...
    return (
        <article className="grid sm:grid-cols-4 grid-cols-3 gap-4 bg-transparent overflow-hidden py-6 border-t w-full pr-3">
            <div className="px-2 py-1 sm:col-span-3 col-span-2">
                <a href={`/blog/${post.slug}`} className="block">
                    
                    <header>
                        <div className="py-1 mt-2">
                            <img src={author?.profile_image} alt={author?.name} className=" h-8 w-8 rounded-full mr-3 hidden sm:inline-block" />
